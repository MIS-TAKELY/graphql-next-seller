...
type AuthStep = "SIGN_IN" | "SIGN_UP" | "PHONE_OTP" | "PHONE_NUMBER" | "EMAIL_SENT" | "FORGOT_PASSWORD_INPUT" | "FORGOT_PASSWORD_OTP" | "FORGOT_PASSWORD_RESET";

export default function SellerUnifiedAuth() {
    ...
    // Handle step transitions and redirects based on session state
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        if ((step === "PHONE_OTP" || step === "FORGOT_PASSWORD_OTP") && timer > 0) {
    ...
    const handleSignUp = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const { error } = await signUp.email({
                ...
                firstName,
                lastName,
                // username logic is handled by plugin or can be generated here if needed
                // buyer explicit generation: username: email.split("@")[0] + "_" + Math.random().toString(36).slice(-5),
                // Since seller auth logic was failing on username, let's include it to be safe, mimicking buyer flow
                username: (email.split("@")[0] + "_" + Math.random().toString(36).slice(-5)).toLowerCase(),
            } as any);
    ...
    const renderSignIn = () => (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="text-left">
                <h2 className="text-2xl font-bold tracking-tight text-foreground">Seller Login</h2>
                <p className="mt-2 text-sm text-muted-foreground">Sign in to manage your shop</p>
            </div>
            <form onSubmit={handleSignIn} className="space-y-4">
                <div className="space-y-2">
                    <Label htmlFor="identifier_login">Email or Username</Label>
                    <Input id="identifier_login" type="text" required value={identifier} onChange={(e) => setIdentifier(e.target.value)} placeholder="name@example.com or shop123" />
                </div>
                <div className="space-y-2">
            ...
            <div className="text-center text-sm">
                ...
                <button onClick={() => setStep("SIGN_UP")} className="font-medium text-primary hover:underline">Register now</button>
            </div>
        </div>
    );
    ...
    const renderSignUp = () => (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="text-left">
                <h2 className="text-2xl font-bold tracking-tight text-foreground">Become a Seller</h2>
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
            ...
            <form onSubmit={handleForgotPasswordSendOtp} className="space-y-4">
                <div className="space-y-2">
                    ...
                    <Input id="forgot_identifier" type="text" required value={forgotPasswordIdentifier} onChange={(e) => setForgotPasswordIdentifier(e.target.value)} placeholder="name@example.com or phone" />
                </div>
                <Button type="submit" className="w-full" disabled={loading}>
                    {loading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
                    Send OTP
                ...
                <div className="text-center">
                    <button type="button" onClick={() => setStep("SIGN_IN")} className="text-sm text-muted-foreground hover:text-primary">Back to Sign In</button>
                </div>
            </form>
        </div>
    );
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <form onSubmit={handleResetPassword} className="space-y-4">
                ...
                <div className="space-y-2">
                    ...
                    <div className="relative">
                        <Input id="confirm_password" type={showConfirmPassword ? "text" : "password"} required value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="••••••••" className="pr-10" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 bg-white/80 dark:bg-slate-950/80 border border-blue-100/50 dark:border-blue-900/20 rounded-3xl shadow-2xl shadow-blue-500/10 backdrop-blur-2xl transition-all duration-300 min-h-[600px]">
                ...
                <div className="hidden md:flex flex-col justify-center items-center bg-blue-50/50 dark:bg-slate-900/50 p-10 relative overflow-hidden">
                    <div className="relative z-10 flex flex-col items-center text-center">
                        ...
                        <h1 className="text-2xl font-bold text-foreground mb-3">Seller Dashboard</h1>
                        <p className="text-muted-foreground text-sm max-w-xs">
                            Manage your shop, products, and orders efficiently.
                        </p>
                    </div>
