...
const TEMPLATES = {
    VERIFICATION: {
        subject: "Verify your email address - Vanijay",
        text: (ctx: TemplateContext) => `Verify your email by clicking here: ${ctx.url}`,
        html: (ctx: TemplateContext) => `
      <div style="font-family: sans-serif; padding: 20px; border: 1px solid #eee; border-radius: 10px; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #333;">Welcome to Vanijay!</h2>
        <p>Hi ${ctx.name || "there"},</p>
        <p>Please verify your email address to get started. Click the button below:</p>
    ...
    PASSWORD_RESET: {
        ...
        text: (ctx: TemplateContext) => `You requested a password reset. Use this link: ${ctx.url}`,
        html: (ctx: TemplateContext) => `
      <div style="font-family: sans-serif; padding: 20px; border: 1px solid #eee; border-radius: 10px; max-width: 600px; margin: 0 auto;">
